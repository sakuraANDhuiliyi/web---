...
class ArticleService {
  // 批量替换url
  async updateUrl() {
    const sourcePrefix = process.env.ARTICLE_COVER_OLD_PREFIX || "http://img.example.com/";
    const targetPrefix = process.env.ARTICLE_COVER_NEW_PREFIX || "http://localhost:9000/blog-images/";
    let res = await Article.findAll();
    res.forEach(async (v) => {
      if (typeof v.dataValues.article_cover === "string" && v.dataValues.article_cover.includes(sourcePrefix)) {
        v.dataValues.article_cover = v.dataValues.article_cover.replace(sourcePrefix, targetPrefix);
      }
