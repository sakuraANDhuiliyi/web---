...
export default defineConfig(({ mode }) => {
  ...
  const blogImagesProxyTarget = env.VITE_PROXY_BLOG_IMAGES || "http://localhost:9000/blog-images";
  const musicApiProxyTarget = env.VITE_PROXY_MUSIC_API || "http://localhost:3000";
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // minio 代理
        "/blog-images": {
          target: blogImagesProxyTarget,
          changeOrigin: true,
          rewrite: (path) => path.replace(/^\/blog-images/, ""),
        },
        "/wapi": {
          // 网易云的音乐代理
          target: musicApiProxyTarget,
          changeOrigin: true,
        ...
        "/music": {
          ...
          rewrite: (path) => path.replace(/^\/music/, ""), // 去掉 /api 前缀
          secure: false, // 如果是 HTTPS 地址，设置为 false
        },
      },
    },
    // 打包输出
