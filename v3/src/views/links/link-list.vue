...
const contactQq = import.meta.env.VITE_CONTACT_QQ || "";
const linkApplyText = import.meta.env.VITE_LINK_APPLY_TEXT || "欢迎提交友链申请";
const defaultSiteAvatar = import.meta.env.VITE_DEFAULT_SITE_AVATAR || "";
let observe;
let box;
    ...
    <el-card class="!m-[3px] !p-[10px]">
      <el-descriptions :column="1">
        ...
        <el-descriptions-item label="博客链接"
          ><span v-copy="siteUrl" class="!cursor-pointer">{{ siteUrl }}</span>
        </el-descriptions-item>
        <el-descriptions-item v-if="contactQq" label="QQ">
          <span v-copy="contactQq" class="!cursor-pointer">{{ contactQq }}</span>
        </el-descriptions-item>
        <el-descriptions-item>
        ...
        <el-col :xs="24" :sm="8" v-for="(item, index) in linksList" :key="item.id">
          <el-card class="card-hover animate__animated animate__fadeIn">
            <div
              ...
              :style="{
                zIndex: 1,
                backgroundImage: `url(${item.site_avatar || defaultSiteAvatar})`,
              }"
