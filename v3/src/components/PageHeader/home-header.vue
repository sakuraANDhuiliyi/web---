...
const initOneSentence = async () => {
  fetch("https://api.vvhan.com/api/ian/rand?type=json")
    ...
    .catch((err) => {
      console.log("一言获取失败", err);
      saying.value = ["今天也要开心哦！"];
    });
};
...
const getBgCover = computed(() => {
  ...
  // 做一个根据路由来判断判断页面背景图片
  let url;
  let myUrl = "https://10wallpaper.com/wallpaper/1366x768/1609/Macro_piano_abstract_art-2016_Music_HD_Wallpaper_1366x768.jpg";
